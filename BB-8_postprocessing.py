...
def parse_args():
    ...
    parser.add_argument('--input_folder', '-i', type=str, action='store',
                        help='input_dir AND output_dir (has to be the output_dir used by BB-8.py)')
...
def write_fasta_file(args, path, adapter_dict, reads):
    ...
    for name, sequence in reads.items():
        ...
        if undirectional:
            direction = '+'
        elif plus_list_name[0] != minus_list_name[0]: ## Michael Note: Can lose reads if list name is not the same
        ...
        name += '_' + str(len(seq))
        if direction == '+': ## Michael Edit: added tag to header to refelct postive strand was sequenced
            out.write('>%s_FORWARD\n%s\n' %(name, seq))
            outa.write('%s_FORWARD\t%s\t%s\n' % (name, ada1, ada2))
            out5.write('>%s_FORWARD\n%s\n' %(name, mm.revcomp(sequence[:plus_positions[0]])))
            out3.write('>%s_FORWARD\n%s\n' %(name, sequence[minus_positions[0]:]))

        elif direction == '-': ## Michael Edit: added tag to header to refelct negative strand was sequenced
            out.write('>%s_REVERSE\n%s\n' %(name, mm.revcomp(seq)))
            outa.write('%s_REVERSE\t%s\t%s\n' % (name, mm.revcomp(ada2), mm.revcomp(ada1)))
            out3.write('>%s_REVERSE\n%s\n' %(name, mm.revcomp(sequence[:plus_positions[0]+40])))
            out5.write('>%s_REVERSE\n%s\n' %(name, sequence[minus_positions[0]:]))
